import { createSupabaseServerClient } from "@/lib/supabase/server";
import { XCircle } from "lucide-react";
import Image from "next/image";
import { SignOutButton } from "@/app/pending-approval/sign-out-button";

export const metadata = { title: "Application Not Approved — Needlebook" };

export default async function RejectedPage() {
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();

  let studioName: string | null = null;
  let rejectionReason: string | null = null;

  if (user) {
    const { data } = await supabase
      .from("profiles")
      .select("studio_name, rejection_reason")
      .eq("id", user.id)
      .single();
    studioName = data?.studio_name ?? null;
    rejectionReason = data?.rejection_reason ?? null;
  }

  return (
    <div className="min-h-screen bg-[var(--nb-bg)] flex flex-col items-center justify-center px-4 py-10">
      <div className="w-full max-w-sm bg-[var(--nb-card)] rounded-2xl border border-[var(--nb-border)] shadow-sm px-8 py-8 text-center">
        <div className="flex justify-center mb-5">
          <Image
            src="/logo.png"
            alt="Needlebook"
            width={200}
            height={200}
            style={{ height: "70px", width: "auto" }}
            priority
          />
        </div>

        <div className="size-12 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-5">
          <XCircle size={22} className="text-red-600" />
        </div>

        <h1 className="text-lg font-semibold text-[var(--nb-text)] mb-2">
          Your application was not approved
        </h1>

        {studioName && (
          <p className="text-sm font-medium text-[var(--nb-text-2)] mb-3">{studioName}</p>
        )}

        {rejectionReason && (
          <div className="text-sm text-[var(--nb-text-2)] bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-5 text-left">
            <span className="font-medium text-red-700">Reason: </span>
            {rejectionReason}
          </div>
        )}

        <p className="text-sm text-[var(--nb-text-2)] leading-relaxed mb-5">
          If you think this is a mistake, contact us at{" "}
          <a
            href="mailto:saltedslightly@gmail.com"
            className="text-[#7C3AED] hover:underline"
          >
            saltedslightly@gmail.com
          </a>
        </p>

        <SignOutButton />
      </div>

      <p className="mt-6 text-xs text-[var(--nb-text-2)]">© {new Date().getFullYear()} Needlebook</p>
    </div>
  );
}
