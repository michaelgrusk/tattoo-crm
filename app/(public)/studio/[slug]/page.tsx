...
import { FlashLightbox } from "./_components/flash-lightbox";
...
export default async function StudioPage({
  ...
  try {
    ...
    let flashPieces: { id: string; title: string; description: string | null; price: number | null; size_guidance: string | null; placement_notes: string | null; image_url: string | null; status: string; repeatable: boolean }[] = [];
    if (flashEnabled) {
      const { data: flashData } = await supabase
        .from("flash_pieces")
        .select("id, title, description, price, size_guidance, placement_notes, image_url, status, repeatable")
        .eq("user_id", profile.id)
        .eq("status", "available")
